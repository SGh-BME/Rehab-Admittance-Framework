...
@dataclass
class AANParams:
    # --- Engagement from force magnitude ---
    # Below F0: essentially not engaged; above F1: fully engaged
    F0_N: float = 0.5   # should be >= force_deadzone_N
    F1_N: float = 2.0   # "clearly engaging" force

    # --- Task difficulty from target error magnitude ---
    e_ref_m: float = 0.05  # meters; error at which w_e saturates to 1

    # --- Variable stiffness bounds (N/m) ---
    Kx_min: float = 2.0
    Ky_min: float = 2.0
    Kx_max: float = 25.0
    Ky_max: float = 25.0

    # --- Optional variable damping bounds (N·s/m) ---
    # Set B*_max = B*_min to disable damping modulation.
    Bx_min: float = 40.0
    By_min: float = 40.0
    Bx_max: float = 40.0
    By_max: float = 40.0
...
def compute_engagement_weight(Fx: float, Fy: float, F0_N: float, F1_N: float) -> float:
    """Smooth engagement weight w_F in [0,1] from force magnitude."""
...
def compute_error_weight(x: float, y: float, x_d: float, y_d: float, e_ref_m: float) -> float:
    """Task difficulty weight w_e in [0,1] from distance-to-target."""
    e = math.hypot(x - x_d, y - y_d)
    denom = max(e_ref_m, 1e-9)
    return sat01(e / denom)


def lowpass_need(need_raw: float, aan_state: AANState, alpha: float, deadband: float) -> float:
    """Low-pass filter for need with deadband."""
...
def schedule_KB_from_need(need: float, aan_params: AANParams) -> tuple[float, float, float, float]:
    """Return (Kx_eff, Ky_eff, Bx_eff, By_eff) from filtered need."""
...
def step_target_admittance_AAN(
    ...
    """
    Engagement-driven AAN target admittance via variable stiffness (and optional damping).

    Core dynamics (per axis):
        M * x_ddot + B(t) * x_dot + K(t) * (x - x_d) = F_ext

    AAN design:
        need_raw = w_F(force) * w_e(error)
        need_f   = LPF(need_raw)
        K(t) = K_min + (K_max - K_min) * need_f
        B(t) = B_min + (B_max - B_min) * need_f   (optional)
    ...
    # --- Compute need (engagement × difficulty) ---
    wF = compute_engagement_weight(Fx, Fy, aan_params.F0_N, aan_params.F1_N)
    wE = compute_error_weight(adm_state.x, adm_state.y, x_d, y_d, aan_params.e_ref_m)
    need_raw = wF * wE
    need_f = lowpass_need(need_raw, aan_state, aan_params.need_alpha, aan_params.need_deadband)
